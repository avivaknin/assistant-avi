...
const App: React.FC = () => {
  ...
  const handleSendMessage = useCallback(async (text: string, isExpansion = false) => {
    ...
    if (!isExpansion && messages.length === 0) {
        ...
        // This is a follow-up in a general chat, clear origin if it somehow exists
        setChatOrigin(null);
    }
    ...
    const fullAnswerMatch = fullResponse.match(/<fullAnswer>([\s\S]*?)<\/fullAnswer>/);

    const summary = summaryMatch ? summaryMatch[1].trim() : '';
    const fullAnswer = fullAnswerMatch ? fullAnswerMatch[1].trim() : fullResponse;
    const hasCustomSummary = !!summary;
    ...
    setMessages((prev) =>
      prev.map((msg) =>
        msg.id === modelMessageId ? { 
            ...msg, 
            isStreaming: false, 
            text: hasCustomSummary ? summary : fullAnswer, 
            fullText: hasCustomSummary ? fullAnswer : undefined,
            sources 
        } : msg
      )
    );
  ...
  const handleSubCategoryClick = (subCat: SubCategory) => {
    if (subCat.linkedCategoryId) {
      ...
      if (linkedCategory) {
        ...
        setSelectedSubCategory(null); // Reset subcategory, as we're navigating to a new category view
        return;
      }
    }
