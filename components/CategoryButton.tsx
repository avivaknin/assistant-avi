  ...
  return (
    <div className="relative w-full h-full">
      <button
        ...
        disabled={isEditing}
        className={`w-full h-full flex flex-col items-center justify-center text-center p-6 border-2 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 ease-in-out transform hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-opacity-50 ${colors.bg} ${colors.border} ${colors.hoverBorder} ${isEditing ? 'opacity-75' : ''}`}
      >
        {IconComponent && (
        ...
        <span className="font-semibold text-gray-800 text-2xl h-16 flex items-center justify-center">{text}</span>
      </button>
